...
class STAM4Rec(Model):
    def __init__(self, placeholders, data_config, **kwargs):
        ...
        self.A_new = tf.sparse_placeholder(tf.float32, shape=(self.n_users+self.n_items, self.n_users+self.n_items), name='alpha_sp')
        ...
        self.stam_weights = self.get_stam_weights()
    ...
    def get_stam_weights(self):
        ...
        stam_weights = tf.concat([stam_weights, stam_weights_batch], 0)
        print("stam_weights: ", stam_weights)
        self.stam_weights = tf.SparseTensor(indices=self.indices,
                                            values=tf.squeeze(tf.reshape(stam_weights, [1,-1])),
